...
int16_t ADC_to_distance_cm(uint16_t adc_val)//ADC를 CM로 환산하기
{
	...
	return (int8_t)distance;//실제 거리 자체는 값이 크지 않기(약 10cm) 때문에 8비트로 돌아옴
}

int main(void)
{
	char buf[32];//문자열을 시리얼로 보내기 위해
	...
	while (1)
	{
		uint16_t adc_raw = ADC_read(1); // PF1 = ADC1

		if (adc_raw <= SENSOR_ERROR_MIN || adc_raw >= SENSOR_ERROR_MAX)
		{
			Put("DISTANCE_OUT\n"); // 비정상 센서 데이터 예외처리: 거리 계산 대신 에러 메시지 출력
		}
		else
		{
			int8_t distance_cm = ADC_to_distance_cm(adc_raw);
